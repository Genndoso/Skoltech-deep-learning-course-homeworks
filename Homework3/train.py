...
class GAN(pl.LightningModule):
    def __init__(
        ...
        if self.truncation_trick:
            ...
            #pass
            self.val_noise.data = torch.FloatTensor(stats.truncnorm.rvs(-1, 1, size=(len(self.train_dataset), self.noise_channels))).to(device=self.device)
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        noise  = torch.FloatTensor(torch.randn((imgs.shape[0], self.noise_channels))).to(device=self.device)


        fake_scores = self.dis(self.gen(noise, labels), labels)
        if  optimizer_idx == 0:
            loss = self.loss(fake_scores)
        else:
            loss = self.loss(fake_scores, self.dis(imgs, labels))
