    ...
    def forward(self, fake_scores, real_scores=None):
        if real_scores is None:
            ...
            if self.loss_type == 'non_saturating':
                loss = -torch.mean(torch.log(torch.sigmoid(fake_scores)))
            else:
            ...
            #pass
            if self.loss_type == 'non_saturating':
                prob_r = torch.sigmoid(real_scores)
                prob_f = 1 - torch.sigmoid(fake_scores)
                loss = - torch.mean(torch.log(prob_r)) - torch.mean(torch.log(prob_f))
            else:
                loss = -torch.mean(-F.relu(1 - real_scores)) - torch.mean(-F.relu(1 + fake_scores))
...
class ValLoss(nn.Module):
    ...
    def calc_fid(real_features, fake_features):
        ...
        fid = np.linalg.norm(np.mean(real_features, axis=1) - np.mean(fake_features, axis=1))**2
        fid += np.trace(np.cov(real_features) + np.cov(fake_features) - 2 * linalg.sqrtm(np.dot(np.cov(real_features), np.cov(fake_features))))
        return fid
        
    @staticmethod
    def calc_is(fake_probs):
        #return # TODO (2 points)
        return np.exp( (fake_probs * np.log( fake_probs / fake_probs.mean(axis=0) ) ).sum(axis=1).mean())
